...
const TransactionsPage = () => {
    ...
    const formatAmount = (amount: number, status: string) => {
        const colorClass = status === 'completed' ? 'text-[#04802E]' :
            status === 'pending' ? 'text-[#DD900D]' :
                status === 'refunded' ? 'text-[#0D5EBA]' : 'text-[#CB1A14]';
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-y border-gray-50 bg-[#F9FAFB]">
                                <th className="px-6 py-4 text-[13px] font-bold text-gray-500">Transaction ID</th>
                                <th className="px-6 py-4 text-[13px] font-bold text-gray-500">Customer Name</th>
                                <th className="px-6 py-4 text-[13px] font-bold text-gray-500">Amount</th>
                                <th className="px-6 py-4 text-[13px] font-bold text-gray-500">Processed by</th>
                                <th className="px-6 py-4 text-[13px] font-bold text-gray-500">Status</th>
                                <th className="px-6 py-4 text-[13px] font-bold text-gray-500">Date</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-50">
                            {transactionData.map((txn) => (
                                <tr key={txn.id} className="hover:bg-gray-50 transition-colors">
                                    <td className="px-6 py-5 text-[14px] text-[#6C6C6C]">{txn.id}</td>
                                    <td className="px-6 py-5 text-[14px] font-medium text-gray-900">{txn.customerName}</td>
                                    <td className="px-6 py-5 text-[14px]">
                                        {formatAmount(txn.amount, txn.status)}
                                    </td>
                                    <td className="px-6 py-5 text-[14px] text-[#6C6C6C] font-medium">{txn.processedBy}</td>
                                    <td className="px-6 py-5">
                                        <span className={`px-4 py-1.5 rounded-full text-[12px] font-medium border ${getStatusStyles(txn.status)} capitalize`}>
                                            {txn.status}
                                        </span>
                                    </td>
                                    <td className="px-6 py-5 text-[14px] text-[#6C6C6C]">{formatDate(txn.date)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
