...
export const EarningsTableSection: React.FC<EarningsTableProps> = ({
    ...
    const [activeTab, setActiveTab] = useState<"transaction" | "onboarding">("transaction");
    const [searchValue, setSearchValue] = useState("");
    const [dateValue, setDateValue] = useState("08-05-2026");
    const [currentPage, setCurrentPage] = useState(1);

    const rawData = activeTab === "transaction" ? MOCK_TRANSACTION_DATA : MOCK_ONBOARDING_DATA;
            ...
            <div className="border border-[#F2F4F7] bg-white p-2.5 rounded-[14px] mb-6">
                <div className="bg-[#F7F7F7] p-1.5 rounded-[10px] inline-flex w-full sm:w-auto">
                    <button
                        ...
                        className={`flex-1 sm:flex-initial px-6 py-2.5 rounded-[8px] text-[13px] transition-all ${activeTab === "transaction"
                    ...
                    <button
                        ...
                        className={`flex-1 sm:flex-initial px-6 py-2.5 rounded-[8px] text-[13px] transition-all ${activeTab === "onboarding"
                                ? "bg-white text-[#024E44] font-bold shadow-sm"
                                : "text-[#6C6C6C]"
                            }`}
                    >
            ...
            <div className="bg-white rounded-[16px] border border-[#E5E7EB] shadow-sm overflow-hidden">
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-[#F9FAFB] border-b border-[#F3F4F6] text-[12px] font-semibold text-[#6C6C6C]">
                                {activeTab === "onboarding" && <th className="py-3.5 px-6"></th>}
                                <th className="py-3.5 px-6">Business Name</th>
                                {activeTab === "onboarding" && <th className="py-3.5 px-6">KYC Level</th>}
                                <th className="py-3.5 px-6">Earning Type</th>
                                {activeTab === "transaction" && <th className="py-3.5 px-6">Balance Before</th>}
                                {activeTab === "onboarding" && <th className="py-3.5 px-6">Balance After</th>}
                                <th className="py-3.5 px-6">Earnings</th>
                                <th className="py-3.5 px-6">Balance After</th>
                                <th className="py-3.5 px-6">Status</th>
                                {activeTab === "transaction" && <th className="py-3.5 px-6">Date</th>}
                            </tr>
                        </thead>
                                ...
                                paginatedData.map((item) => (
                                    <tr key={item.id} className="hover:bg-[#F9FAFB]/60 transition-colors">
                                        <td className="py-4 px-6">
                                            <div className="flex items-center gap-3">
                                                ...
                                                {activeTab != "onboarding" && (
                                                    <td className="text-[#6C6C6C]">{item.businessName}</td>
                                                )}                                            </div>
                                        </td>

                                        {activeTab === "onboarding" && (
                                            <td className="py-4 px-6 text-[#6C6C6C]">{item.businessName}</td>
                                        )}
                                        {activeTab === "onboarding" && (
                                            <td className="py-4 px-6">
                                        ...
                                        {activeTab === "transaction" && (
                                            <td className="py-4 px-6 text-[#6C6C6C]">{item.balanceBefore}</td>
                                        )}

                                        {activeTab === "onboarding" && (
                                            <td className="py-4 px-6 text-[#6C6C6C]">{item.balanceAfter}</td>
                                        )}

                                        <td className="py-4 px-6 text-[#6C6C6C]">{item.earnings}</td>
                                        <td className="py-4 px-6 text-[#6C6C6C]">{item.balanceAfter}</td>
                ...
                <div className="p-4 border-t border-[#F3F4F6] bg-white flex flex-col sm:flex-row justify-between items-center gap-3 text-xs">
                    <span className="text-[#6B7280] text-[12px] font-medium">
                        Showing {startIndex} to {endIndex} of {totalItems} inactive businesses
                    </span>
