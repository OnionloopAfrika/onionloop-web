      ...
      <table className="w-full text-left border-collapse table-auto">
        <thead>
          <tr className="border-b border-gray-100 bg-[#F7F7F7]">
            <th className="px-4 py-3 text-[15px] font-semibold text-[#6C6C6C] whitespace-nowrap">
              Groups
            </th>
            <th className="px-4 py-3 text-[15px] font-semibold text-[#6C6C6C] whitespace-nowrap">
              Location
        ...
        <tbody className="divide-y divide-gray-100">
          {filteredData?.map((loc) => {
            ...
            return (
              <tr
                key={loc.id}
                className="hover:bg-gray-30/50 transition-colors"
              >
                ...
                <td className="px-4 py-3 whitespace-nowrap">
                  <div className="flex flex-col">
                    <span className="text-[15px] font-normal text-[#6C6C6C]">
                      {loc.revenue}
                    </span>
                    <span
                      className={`text-xs font-normal flex items-center gap-0.5 ${loc.isTrendUp ? "text-green-600" : "text-red-600"}`}
                    >
                      {loc.isTrendUp ? "↑" : "↓"}
                      {loc.trend}
                    </span>
                ...
                <td className="px-4 py-3 whitespace-nowrap">
                  <span
                    className={`px-4 py-1 border rounded-full text-xs font-normal ${styles.badge}`}
                  >
                    {loc.status}
                  </span>
                </td>
