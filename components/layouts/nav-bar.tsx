...
import { ProfileIcon, SettingsIcon, LogoutIcon, CopyIcon, SupportIcon, StaffIconSolid } from '../icons/svgs';
    ...
    <>
      <nav className="w-full bg-white border-b border-gray-100 shadow-[0_1px_4px_rgba(0,0,0,0.06)] sticky top-0 z-[60]">
        <div className="flex items-center h-16 md:h-18 px-4 md:px-7 max-w-360 mx-auto justify-between">
          <Link href="/overview" className="flex items-center gap-2 no-underline shrink-0">
            <OnionloopIcon />
          </Link>
          ...
          <div className="hidden md:flex items-center gap-2 shrink-0">
            ...
            >
              <div className="flex items-center gap-2.5 ml-2 pl-1.5 pr-2.5 py-1.5 rounded-full border border-gray-200 cursor-pointer hover:border-gray-300 transition-colors duration-150">
                <Avatar businessName={businessName} avatarUrl={avatarUrl} userName={userName} />
          ...
          <div className="md:hidden bg-[#F9FAFB] min-h-[calc(100vh-64px)] p-4 animate-in slide-in-from-right duration-300 overflow-y-auto relative">
            <div className={`${"absolute top-[calc(0)] right-0 w-full md:w-96 bg-[#F9FAFB] md:bg-white rounded-xl md:shadow-[0_8px_30px_rgba(0,0,0,0.12)] md:border border-gray-100 p-4 flex flex-col items-center z-100"}`}>
              <div className="bg-white border border-gray-100 rounded-2xl p-6 w-full flex flex-col items-center mb-6 shadow-sm">
                ...
                <h3 className="text-[16px] font-bold text-[#1A1A1A] mb-1">{businessName}</h3>

                <div className="flex items-center gap-2 text-[#666666]">
                  <span className="text-[13px] font-normal">yettymamalounge.onionloopafrika.com</span>
              ...
              <div className="w-full bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
                {[
                  { id: "my-profile", label: "My Profile", icon: <ProfileIcon />, isDestructive: false },
                  { id: "account-settings", label: "Account Settings", icon: <SettingsIcon />, isDestructive: false },
                  { id: "staff-app-settings", label: "Staff App Settings", icon: <StaffIconSolid />, isDestructive: false },
                  { id: "help-and-support", label: "Help & Support", icon: <SupportIcon />, isDestructive: false },
                  { id: "logout", label: "Log Out", icon: <LogoutIcon />, isDestructive: true },
                ].map((item, idx) => (
                  <Link
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className={`w-10 h-10 rounded-xl border border-gray-50 flex items-center justify-center shrink-0 ${item.isDestructive ? 'text-[#D32F2F]' : 'text-[#04907E]'}`}>
                        {item.icon}
                      </div>
                      <span className={`text-[14px] ${item.isDestructive ? 'text-[#D32F2F] font-medium' : 'text-[#363636] font-medium'}`}>
                        {item.label}
                      </span>
                    </div>
        ...
        <div className="fixed inset-0 z-[70] md:hidden">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-[280px] bg-white flex flex-col animate-in slide-in-from-left duration-300">
            <div className="p-4 flex items-center justify-between border-b border-gray-100">
              ...
              >
                <Avatar businessName={businessName} avatarUrl={avatarUrl} userName={userName} size="w-10 h-10" />
                <div className="flex flex-col">
                  <span className="text-[14px] font-bold text-gray-900 leading-tight">{businessName}</span>
                  <span className="text-[12px] text-gray-500">{userName}</span>
                </div>
              </div>
              <button onClick={() => setIsMenuOpen(false)} className="p-1.5 rounded-full border border-gray-100 text-gray-400">
              ...
              {NAV_ITEMS.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-[14.5px] transition-colors ${isActive ? "bg-[#E6F0EE] text-[#024E44] font-bold" : "text-gray-600 hover:bg-gray-50"
                      }`}
                  >
                    <div className="w-8 h-8 rounded-lg bg-gray-50 flex items-center justify-center">
                      {isActive ? item.activeIcon(ACTIVE_COLOR) : item.icon("#6B7280")}
                    </div>
                    {item.label}
                  </button>
                );
...
function Avatar({ businessName, avatarUrl, userName, size = "w-9 h-9" }: { businessName: string, avatarUrl?: string, userName: string, size?: string }) {
  return (
    <div className={`${size} rounded-full overflow-hidden shrink-0 bg-[#d6f0e6] flex items-center justify-center border border-gray-100`}>
      {avatarUrl ? (
        <img src={avatarUrl} alt={userName} className="w-full h-full object-cover" />
      ) : (
