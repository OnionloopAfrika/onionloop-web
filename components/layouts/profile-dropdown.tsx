  ...
  return (
    ...
    >
      <div className="flex flex-col items-center mb-6 px-6 w-full text-center">
        ...
        <h3 className="text-[14px] font-semibold text-[#363636] leading-tight">
          {businessName}
        </h3>
      ...
      <div className="w-full flex flex-col gap-1">
        {menuItems.map((item, idx) => (
          ...
          >
            ...
            <span
              className={`text-[12px] font-normal ${item.isDestructive ? "text-[#D32F2F]" : ""} ${item.isDestructive && active === item.id ? "text-[#D32F2F] font-bold" : ""} ${active === item.id ? " text-[#04907E] font-bold" : ""}`}
            >
              {item.label}
            </span>
          </Link>
