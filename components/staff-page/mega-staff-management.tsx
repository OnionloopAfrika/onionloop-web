...
interface StaffManagementProps {
  leaveRequestPath?: string;
}

export default function StaffManagement({
  leaveRequestPath = "/super-admin/staff/leave-request",
  ...
  const [selectedStaffOption, setSelectedStaffOption] = useState<string | null>(
    "activate",
  );
  ...
  const handleAction = (
    type: "profile" | "edit" | "deactivate" | "remove",
    staff: Staff,
  ) => {
...
<<<<<<< HEAD
  const handleToggleStatus = () => {
    ...
    setStaffs((prev) =>
      prev.map((s) =>
        s.id === selectedStaff.id ? { ...s, status: newStatus } : s,
      ),
=======
    const closeModal = () => {
        setModalType(null);
  ...
  return (
    <div className="min-h-screen">
      <div className="flex flex-col md:flex-row gap-4 justify-between items-start mb-6">
        ...
        <div className="flex gap-3 w-full md:w-auto justify-end">
          <button
            onClick={() => router.push(`/${subdomain}${leaveRequestPath}`)}
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 md:gap-4 gap-2 py-4">
        {[
          ...
          },
          {
            ...
            count: 4,
            sub: "Using staff app",
            color: "bg-green-100 text-[#04802E]",
            icon: <TimerClock />,
          ...
          <div
            key={i}
            className={`bg-white p-4 md:p-6 rounded-xl border border-gray-100 shadow-sm ${stat.className}`}
          >
            <div
              className={`w-10 h-10 rounded mb-4 flex items-center justify-center ${stat.color}`}
            >
              {stat.icon}
            </div>
            <h3 className="text-[16px] md:text-[28px] font-semibold">
              {stat.count}
            </h3>
            <p className="text-gray-500 text-sm font-normal">{stat.label}</p>
            <span
              className={`text-[10px] px-2 py-0.5 rounded-full mt-2 inline-block ${stat.color} bg-opacity-20 font-medium`}
            >
              {stat.sub}
            </span>
          </div>
      ...
      <div className="my-0 md:my-6">
        <h3 className="text-[14px] font-normal text-[#131313] mb-2 md:hidden">
          All Staffs
        </h3>
        <div className="bg-white p-4 md:rounded-xl rounded-t-xl flex flex-row-reverse md:flex-row items-center justify-between border border-gray-100 gap-4">
          <div className="w-full md:w-1/3 flex gap-2 items-center">
            ...
            <div className="flex justify-between items-start mb-4">
              <div className="flex gap-3">
                <div className="relative">
                  ...
                  />
                  <div
                    className={`absolute bottom-0 right-0 w-3 h-3 border-2 border-white rounded-full ${staff.status === "Active" ? "bg-green-500" : "bg-red-500"}`}
                  />
                </div>
                <div>
                  <h4 className="font-medium text-[#131313]">{staff.name}</h4>
            ...
            <div className="bg-[#F7F7F7] p-4 rounded-lg flex justify-between items-center mb-4">
              <div>
                <p className="text-[20px] font-bold">
                  {staff.salesToday.toString().padStart(2, "0")}
                </p>
                <p className="text-[10px] text-[#6C6C6C] tracking-wider">
                  Sales Today
                </p>
              </div>
              <span
                className={`text-[10px] px-3 py-1 rounded-full font-semibold ${staff.status === "Active" ? "bg-[#04802E] text-white" : "bg-[#98A2B3] text-white"}`}
              >
                {staff.status}
              </span>
            </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[480px]">
            <thead>
              <tr className="border-b border-gray-100 bg-[#F9FAFB]">
                <th className="p-4 text-sm font-semibold text-gray-500 text-left w-1/2"></th>
                <th className="p-4 text-sm font-semibold text-gray-500 text-left w-1/2">
                  Name
                </th>
                <th className="p-4 text-sm font-semibold text-gray-500 text-left w-1/4">
                  Role
                </th>
                <th className="p-4 text-sm font-semibold text-gray-500 text-left w-1/5">
                  Status
                </th>
                <th className="p-4 text-sm font-semibold text-gray-500 text-right w-[40px]"></th>
              </tr>
            ...
            <tbody>
              {staffs.map((staff) => (
                <tr
                  key={staff.id}
                  className="border-b border-gray-100 last:border-none align-middle"
                >
                  <td className="p-4 pr-2">
                    <img
                      src={staff.image}
                  ...
                  <td className="p-4 pr-2">
                    <div className="flex items-center gap-3">
                      <span className="font-normal text-sm text-[#6C6C6C] truncate max-w-[140px]">
                        {staff.name}
                      </span>
                    </div>
                  </td>
                  ...
                  <td className="p-4 px-2 text-left">
                    <span
                      className={`text-[12px] px-3 py-1 rounded-full font-medium inline-block text-center min-w-[80px] ${staff.status === "Active" ? "bg-[#04802E] text-[#ffffff]" : "bg-[#98A2B3] text-[#F9FAFB]"}`}
                    >
                      {staff.status}
                    </span>
                  </td>
                  <td
                    className="p-4 pl-2 text-right relative"
                    data-menu-container
                  >
                    <button
                      onClick={() =>
      ...
      >
        <div className="py-6">
          ...
          <h2 className="text-[24px] font-bold text-[#04907E] mb-2">
            Invite sent successfully!
          </h2>
          <p className="text-[#6C6C6C] text-[14px] mb-10">
            The invite has been sent to the provided email address
          </p>
          <Button
            variant="primary"
            className="w-full !bg-[#044E49] !py-4"
            onClick={closeModal}
          >
            Done
          </Button>
          ...
          <div className="mb-6">
            ...
            <div className="space-y-3">
              {mockLocations.slice(0, 5).map((loc, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between p-1 hover:bg-gray-50 rounded-xl transition-colors cursor-pointer"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-[#EDE8FC] text-[#7C53FC] font-semibold text-sm flex items-center justify-center flex-shrink-0">
                      IK
      ...
      <Modal
        open={modalType === "all-locations"}
        onOpenChange={() => setModalType("profile")}
        className="max-w-[580px]"
      >
        <div className="max-h-[85vh] overflow-y-auto px-1 font-sans text-center">
          ...
          <p className="text-[#6C6C6C] text-[14px] mb-6">
            They will receive an invite to download the onionloop staff app
          </p>
      ...
      >
        <div className="max-h-[70vh] overflow-y-auto px-1">
          ...
          <div className="grid grid-cols-2 gap-y-6 mb-8 text-[13px]">
            <div>
              <p className="text-[#6C6C6C]">Full Name:</p>
              <p className="font-bold text-[#131313]">{selectedStaff?.name}</p>
            </div>
            <div className="text-right">
              <p className="text-[#6C6C6C]">Status:</p>
              <span
                className={`text-[#fff] font-bold px-3 py-0.5 rounded-full ${selectedStaff?.status === "Active" ? "bg-[#04802E]" : "bg-[#98A2B3]"}`}
              >
                {selectedStaff?.status}
              </span>
            </div>
            <div>
              <p className="text-[#6C6C6C]">Phone Number:</p>
              <p className="font-bold text-[#131313]">{selectedStaff?.phone}</p>
            </div>
            <div className="text-right">
              <p className="text-[#6C6C6C]">Email Address:</p>
              <p className="font-bold text-[#131313] truncate">
                {selectedStaff?.email}
              </p>
            </div>
          </div>
          ...
          <Input
            label="Current Role"
            placeholder="Head of Staff"
            className="mb-8"
          />
          <p className="font-bold text-[14px] text-[#131313] mb-4">
            App Permissions
          </p>
          <div className="grid grid-cols-2 gap-4 mb-10">
            {Object.keys(permissions).map((perm) => (
              <div
                key={perm}
                className="flex justify-between items-center p-4 bg-[#F7F7F7] rounded-xl border border-gray-50"
              >
                <span className="text-[14px] text-[#6C6C6C]">{perm}</span>
                <Switch
                  checked={permissions[perm]}
      ...
      >
        <div className="flex justify-center mb-6">
          <div className="rounded-2xl">
            <AlertOctagon
              size={60}
              color={isReactivating ? "#024E44" : "#DD900D"}
            />
          </div>
        </div>
        <h2
          className={`text-[20px] font-bold mb-3 ${isReactivating ? "text-[#04802E]" : "text-[#DD900D]"}`}
        >
          {isReactivating
      ...
      >
        ...
        <h2 className="text-[20px] font-bold text-[#CB1A14] mb-3">
          Remove Staff Account?
        </h2>
        <p className="text-[#6C6C6C] text-[14px] mb-10">
          Are you sure you want to remove this staff as your staff member?
        </p>
        <div className="flex gap-4">
          <Button
            variant="secondary"
