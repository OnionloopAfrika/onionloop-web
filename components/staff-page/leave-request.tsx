...
const MoreVertical = () => (
    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M10 4.16699V4.17533M10 10.0003V10.0087M10 15.8337V15.842" stroke="#A8A8A8" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
);
                ...
                <div className="flex gap-2 p-1 bg-[#F7F7F7] w-full md:w-fit rounded-full">
                    {["All", "Pending", "Approved", "Rejected"].map((tab) => (
                        <button
                            ...
                            className={`flex-1 md:flex-initial md:px-6 md:py-2 p-[8px_12px] text-[10px] rounded-full md:text-[14px] font-medium transition-all text-center ${activeTab === tab ? "bg-white shadow-sm text-[#024E44] font-semibold" : "text-[#6C6C6C]"
                                }`}
                        >
                            {tab}
                        </button>
                    ...
                    <table className="w-full border-collapse min-w-[520px] md:min-w-full">
                        <thead>
                            <tr className="text-left bg-[#FBFBFB] border-b border-[#F0F0F0]">
                                ...
                                <th className="p-4 md:p-6 text-[14px] font-semibold text-[#6C6C6C] text-left">Employee</th>
                                <th className="p-4 md:p-6 text-[14px] font-semibold text-[#6C6C6C] text-left">Leave Type</th>
                                <th className="p-4 md:p-6 text-[14px] font-semibold text-[#6C6C6C] text-left">Status</th>
                            ...
                            {filteredRequests.map((request) => (
                                <tr key={request.id} className="border-b border-[#F0F0F0] last:border-none hover:bg-gray-50 transition-colors align-middle">
                                    <td className="p-4 md:p-6 pr-2">
                                        <img src={request.avatar} alt="" className="w-10 h-10 rounded-full border border-[#c6c6c6] flex-shrink-0" />
                                    </td>
                                    <td className="p-4 md:p-6 pr-2">
                                        <div className="flex items-center gap-3">
                                            <span className="font-normal text-sm text-[#6c6c6c] truncate max-w-[150px] md:max-w-none">{request.employee}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td className="p-4 md:p-6 px-2 text-left">
                                        <span
                                            className={`px-4 py-1 rounded-full text-[12px] font-medium inline-block text-center min-w-[90px] ${request.status === "Approved"
                                        ...
                                        >
                                            {request.status === "Rejected" ? "Rejection" : request.status}
                                        </span>
                                    </td>
                                    <td className="p-4 md:p-6 pl-2 text-right relative">
                                        ...
                                        {activeMenu === request.id && (
                                            <div ref={menuRef} className="absolute right-4 top-12 w-44 bg-white border border-gray-100 shadow-xl rounded-lg z-20 overflow-hidden text-left">
                                                {request.status === "Approved" ? (
                                                    <div className="p-4 flex items-center gap-2 text-[#008A70] text-sm font-medium bg-gray-50/50">
                                                        <svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <path d="M16.6663 5L7.49967 14.1667L3.33301 10" stroke="#008A70" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
                                                        </svg>
                                                        Approved
                                                    </div>
                                                    ...
                                                    <>
                                                        <button onClick={() => setActiveMenu(null)} className="w-full text-left px-4 py-3 text-sm font-medium text-[#008A70] hover:bg-gray-50 border-b border-gray-50">
                                                            Approve
                                                        </button>
                                                        <button onClick={() => setActiveMenu(null)} className="w-full text-left px-4 py-3 text-sm font-medium text-[#F34139] hover:bg-gray-50">
                                                            Reject
                                                        </button>
                                                    </>
                                                )}
